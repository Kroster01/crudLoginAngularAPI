...
app.get('/users', (req, res) => {
  ...
  connection.query(sql, (error, results) => {
    if (error) {
      res.send('Not result, Error, app.get');
    }
    if (results.length > 0) {
      res.json(results);
    } else {
...
app.get('/users/:id', (req, res) => {
  ...
  connection.query(sql, (error, result) => {
    if (error) {
      res.send('Not result, Error, app.get-ID');
    }
...
app.post('/add', (req, res) => {
  ...
  connection.query(sql, customerObj, error => {
    if (error) {
      res.send('Not result, Error, app.post');
    }
...
app.put('/update/:id', (req, res) => {
  ...
  connection.query(sql, error => {
    if (error) {
      res.send('Not result, Error, app.put');
    }
...
app.delete('/delete/:id', (req, res) => {
  ...
  connection.query(sql, error => {
    if (error) {
      res.send('Not result, Error, app.delete');
    }
...
connection.connect(error => {
  if (error) {
    res.send('Not result, Error, connection.connect');
  }
